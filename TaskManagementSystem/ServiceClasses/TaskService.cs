       ...
       public void CreateTask(TaskManagementSystemDbContext DbContextObj, Models.ViewModels.TaskViewModel TaskObject)
        {
            ...
            if (DbContextObj.Tasks.Where(w => w.Project_Id == TaskObject.Project_Id).Count() > 0)
                {
                TaskModelObj.Id = int.Parse((DbContextObj.Tasks.Where(w => w.Project_Id == TaskObject.Project_Id).Select(s => s.Id).Max() + 1).ToString() + TaskObject.Project_Id.ToString());
